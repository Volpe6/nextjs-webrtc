...
import { getUserMedia } from "@/utils/mediaStream";
        ...
        <div className="flex flex-col justify-start w-1/4 bg-white border-r border-gray-200 px-8 py-6 min-w-[270px]">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex">
              ...
              <ul>
                {callManager.calls.map((call, i) => 
                  !call.isIncoming&&<li key={i} className="flex flex-row items-center space-x-2">
                    <span>chamando: {call.target}</span>
                  </li>
                )}
              </ul>
            </div>
            <div className="flex">
              ...
              <ul>
                {callManager.calls.map((call, i) => 
                  call.isIncoming&&<li key={i} className="flex flex-row items-center space-x-2">
                    <span>recebendo chamada: {call.name}</span>
                    <button
                      className="rounded-md py-2 px-4 bg-blue-500 text-white font-medium focus:outline-none hover:bg-blue-600"
